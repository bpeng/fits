 ...
 * -- static jfree chart image for seleted sites and param (ie) from rest service
 * 
 * baishan 5/10/2012
 **********************************************************/
...
var ldrChartClient = {
    ...
    sparklinePath :'services/sparkline/',
    ...
    showSitesData: function (sites){
        ...
        var sitesLayer = new L.GeoJSON1(sites, {
            ...
            filter: function(feature, layer){            	
                if (feature.properties && feature.properties.code) {
                    if(ldrChartClient.selectedSites && ldrChartClient.selectedSites.length > 0){
                        if(ldrChartClient.selectedSites.indexOf(feature.properties.code) >= 0){//show only selected sites
                            return true;
                        }
            ...
            pointToLayer: function (feature, latlng) {
                ...
                var siteIconUrl = ldrChartClient.sparklinePath + feature.properties.code + "/" + ldrChartClient.selectedParam + "/m.png";
    ...
    openSitePlot:function(e){
        if( e.target &&  e.target.site) {
            ...
            var url = "chart?site=" +  e.target.site + "&param=" + this.selectedParam;
        ...
        if(site && param){
            if(this.iev > 0){//show static chart
                ...
                if(data){
                    ...
                    this.queryChartResultsSite(site, param);
                }
            }
        }
    },
    ...
    showSites: function() {
        ...
        if(sitesData){
            ...
            var url = this.dataUrl + "sites/" + this.selectedRegion + ".json";
    ...
    showSitesDataSelection: function (sites){   	
        if(sites && sites.features && sites.features.length > 0){
            ...
            for (var i = 0, len =  sites.features.length; i < len; i++) {
                ...
                $('#selSites').append('<option value="' + feature.properties.code + '">' + feature.properties.alias + '</option>');
            }
            //show listbox
    ...
    /***
	 * query observation results for sigle site from http
	 * ***/
    queryChartResultsSite:function(site, param){
        var url  = this.dataUrl + "results/site/" + site + "/" + param + ".json";
        jQuery.getJSON( url, function (data) {
            //console.log(JSON.stringify(data));
            ldrChartClient.processPlotDataSite(data,site,param);           
        });
    },  
    
    
    /***
	 * query observation results for selected region from http
	 * ***/
    queryChartResults:function(){
        var url = this.dataUrl + "results/region/" + this.selectedRegion + "/" + this.selectedParam + ".json";  
        var sites = null;    
        if(this.selectedSites && this.selectedSites.length > 0) {
            url = this.dataUrl + "results/region/" + this.selectedRegion + "/" + this.selectedParam + "/" + this.selectedSites  + ".json";
    ...
    /*
	 * make dygraphs chart for selected region and param
	 */
    ...
    //parse, store data and make plot, single series data  
    processPlotDataSite:function(_data, site, param){
        var chartData = this.parsePlotData(_data.results);
        _data.results = chartData;
        var datakey = site + "_" + param;        
        this.allDygraphData[datakey] = _data;
        //if(console) console.log("2 datakey " + datakey) ;
        this.makeDygraphPlot(_data.results, null, _data.param, [_data.site]);
    },
